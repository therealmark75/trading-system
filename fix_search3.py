...
# Find and comment out Search 3 block entirely
old = '''    # ── Search 3: Full-text search for enforcement keywords ──────────────────
...
new = '''    # Search 3 removed - full-text keyword search produces too many false positives
    # (fund filings mentioning ticker as a holding alongside SEC investigation keywords)
    # Litigation Releases (Search 1) and 8-Ks (Search 2) are sufficient and reliable'''

if old in content:
    content = content.replace(old, new)
    with open('scrapers/legal_risk_scraper.py', 'w') as f:
        f.write(content)
    print('✅ Search 3 removed')
else:
